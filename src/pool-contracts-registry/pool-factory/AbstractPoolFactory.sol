...
abstract contract AbstractPoolFactory is AbstractDependant {
    ...
    function _deploy(address poolRegistry, string memory poolType) internal returns (address) {
        // TODO - to control the created spherex proxy, an admin functionallity is needed,
        // default spherex admin is set to msg.sender
        return address(
     ...
     */
    function _deploy2(address poolRegistry, string memory poolType, bytes32 salt) internal returns (address) {
        // TODO - to control the created spherex proxy, an admin functionallity is needed,
        // default spherex admin is set to msg.sender
        return address(
    ...
    function _register(address poolRegistry, string memory poolType, address poolProxy) internal {
        (bool success,) =
            poolRegistry.call(abi.encodeWithSignature("addProxyPool(string,address)", poolType, poolProxy));
     ...
     */
    function _predictPoolAddress(address poolRegistry, string memory poolType, bytes32 salt)
        internal
        view
        returns (address)
    {
        bytes32 bytecodeHash = keccak256(
            abi.encodePacked(
                ...
                abi.encode(AbstractPoolContractsRegistry(poolRegistry).getProxyBeacon(poolType), "")
            )
        );
